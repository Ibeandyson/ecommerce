    ...
    if (trim($id) != '') {
        if(check_allow()){
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                /*SEND AD APPROVE FIREBASE NOTIFICATION TO AUTHOR*/
                $note_title = "Congratulations!!";
                $message = $item_title." has been approved, Make it premium for more visibility.";
                add_firebase_notification("","","",$item_author_id,$product_id,$item_title,"ad_approve",$message);
                sendFCM($message,$item_author_id,$note_title,$sending_type = "one_user");

                /*SEND AD APPROVE FIREBASE NOTIFICATION TO USERS AS NEWSLETTER*/
                $note_title = "Check it out!!";
                $message = $item_title." has been added.";
                sendFCM($message,$item_author_id,$note_title,$sending_type = "all_user");
...
function deleteads()
{
    ...
    if (is_array($_POST['list'])) {
        ...
        $sql2 = "SELECT id,product_name,user_id,screen_shot FROM `".$config['db']['pre']."product` ";
        foreach ($_POST['list'] as $value)
        {
            if($count == 0)
            {
            ...
            if ($result = $con->query($sql2)) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    foreach ($screen_sm as $value)
                    {
                        ...
                        if(file_exists($filename1)){
                            $filename1 = $uploaddir.$value;
                            $filename2 = $uploaddir."thumb/".$value;
                            unlink($filename1);
                            unlink($filename2);
                        }
                    }
                    ...
                    $item_author_id = $row['user_id'];

                    /*SEND AD DELETED FIREBASE NOTIFICATION TO AUTHOR*/
                    $note_title = "Deleted!!";
                    $message = $item_title." has been Deleted.";
                    add_firebase_notification("","","",$item_author_id,$product_id,$item_title,"ad_delete",$message);
                    sendFCM($message,$item_author_id,$note_title,$sending_type = "one_user");
